...
- **运维**：备份/恢复（含便携备份同盘风险提示）、数据目录迁移（复制+哈希校验）、诊断包、健康检测（含磁盘与检索预热）、首次启动检查（数据目录/Provider 配置/健康汇总）、Provider 连通性「API 测试」。
...
| 层 | 命令 | 结果 |
| --- | --- | --- |
| 后端单元/集成 | `pytest -q`（`backend`，非 performance） | **167 passed** |
